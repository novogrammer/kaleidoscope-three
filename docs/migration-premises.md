...
従来の画像処理で使われてきたLena／Lenna画像は採用しない。元画像の再配布条件が明確なfixture向けライセンスではなく、被写体本人も利用終了を望んでいる。fixtureには画像生成AIで作成した架空人物の顔写真を使用する。正面顔の基準画像として`src/assets/fixtures/face-center.jpg`を作成済みであり、生成方法とハッシュは`README.md`に記録する。顔が中央から外れた画像と顔がない画像は、実装上必要になった時点で追加する。
