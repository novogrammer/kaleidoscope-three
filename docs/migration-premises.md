...
`KaleidoscopeUnityHdr`のthree.js版について、移植元から確認できた事実と、実装・検証で維持する設計上の判断を整理する。
...
警告文は、少なくとも強い光や点滅を含むことと、明るい部屋で画面から離れて閲覧することを伝える。安全上重要な内容をブラウザ翻訳だけに依存させないため、日本語と英語を併記し、開始ボタンにも英語を添える。
...
顔メッシュ自体は作成されるが、現在のシーン設定ではデバッグ描画が無効であり、作品の主要出力には使用されていないように見える。three.js版では468点の顔メッシュ全体を再現せず、顔検出中心を使用する。
...
- フェード係数: 検出状態に向けて線形補間
- 半径: フェード係数を直接使用
- 単位長: フェード係数に`EaseOutSine`を適用してから最大係数を乗算
- 顔を見失った場合: 最後に検出した中心をフェードアウト完了まで保持
...
VFX Graphから確認できた主要な値は次のとおり。

- 生成数: `50`個/秒
- 寿命: `20`秒
- 事前更新: `10`秒（`1/60`秒を600ステップ）
- 最大容量: `1000`
- 初期位置: 横幅`20`、厚み`0.1`の上辺
- 初速: 下向きを基準に`0.5–1.0`
- 描画サイズ: `0.25`
- 初期角度: `1–90`度
- 角速度: `-1800–1800`度/秒
- 色: 赤、橙、緑、青、水色寄りの紫、マゼンタ寄りの赤の6色

紙吹雪シェーダーでは、1秒周期の減衰値を`0.05–1.0`へ再マップしてから`50`倍し、基本色へ乗算している。したがって紙吹雪は線形色空間で`1.0`を超える値を生成し、Bloomへ渡す主要な高輝度入力である。

UnityのVFX GraphをWebへ直接移植することはできないため、これらの値を基準に、パーティクルの生成、更新、寿命、配置、色、描画形状をthree.js向けに再構成する。
...
- レンダラー、シーン、入力など、ライフサイクルと状態を持つサブシステムはclassベースで構成する。URL解析や座標変換などの純粋処理は関数として分離する。
...
- カメラ映像のメタデータ取得は最大10秒待機し、映像エラーまたはタイムアウト時はカメラ初期化失敗としてテストパターンへフォールバックする。
- 描画開始後に顔検出処理が例外を投げた場合は検出器を破棄し、顔なし入力へ切り替えて補助表示を継続する。実行中の再試行は行わない。
- 描画更新と顔検出の頻度を分離し、動画顔検出は15fpsで実行して同じ動画フレームを重複して検出しない。
- 描画用カメラ映像は取得解像度を維持し、顔検出時だけ縦横比を保ったまま長辺`320px`の再利用Canvasへ縮小してMediaPipeへ渡す。元映像がそれより小さい場合は拡大しない。
- 開発ビルドではCanvasへの縮小と動画顔検出を合わせた処理時間を計測し、直近60回の最新値、平均値、最大値を60回ごとにコンソールへ出力する。この実測値と15fpsの時間予算`66.7ms`を比較して、検出頻度を確定する。本番ビルドでは計測とログ出力を行わない。
- 前面カメラ映像は描画時に左右反転し、MediaPipeの顔中心も別途左右反転して表示位置と一致させる。
- 15fpsの顔中心を描画フレームごとに指数平滑化し、半減期`100ms`を使用する。初回検出時は非表示位置から移動して見えないよう、平滑化せず直ちに中心を合わせる。
- カメラが利用できない場合は決定的なテストパターンへ切り替え、主レイヤーを非表示にしたまま補助レイヤーと紙吹雪の描画を継続する。MediaPipeの初期化に失敗した場合も画像入力は維持し、同様に補助表示を継続する。

### 開発・検証用の入力

開発中やブラウザ検証では、Webカメラを必須にしない。カメラ権限、実行環境、照明、人物の位置による不安定さを避けるため、URLクエリパラメーターで入力を切り替える。検証のためにソースコードを書き換えない。
...
fixtureの顔検出結果は初期化時に固定せず、カメラと同様に描画ループから最大15fpsで更新する。MediaPipeの実行モードは入力種別に合わせて静止画を`IMAGE`、カメラを`VIDEO`とするが、長辺320px以下の検出用Canvas、頻度制御、結果変換、開発時計測は共通経路を使用する。

具体的なURLパラメーター、既定値、確認用の組み合わせは`README.md`に記載する。元画像と紙吹雪を確認するときは、URLパラメーターで万華鏡変換だけを迂回し、Bloomと最終出力処理は通常経路を使用する。通常利用時のカメラは、開始画面で利用者が明示的に開始した後だけ起動する。

静止画fixtureには、利用条件が明確でリポジトリに配置できる画像だけを使用し、個人の写真や利用条件が不明な画像は含めない。画像生成AIで作成した架空人物による正面顔、画面中央から外れた顔、顔を含まない背景の3種類を用意する。生成方法とハッシュは`README.md`に記録する。
...
- `RenderPipeline`とTSLベースのポストプロセスを使用する。
...
- BloomパラメーターはUnity版を基準にするが、アルゴリズム差があるため数値一致ではなく視覚比較で調整する。
...
- VFX Graph全体を機械的に変換せず、見た目に寄与するパラメーターと更新式を抽出する。
- WebGPUとWebGL 2で共通利用できる実装を優先する。
- パーティクル状態はCPUで更新し、`InstancedMesh`でまとめて描画する。
- パーティクル数はUnity版と同じ容量`1000`を上限とする。
- 再現性のある固定シードを使い、15個/秒、寿命30秒、10秒の事前更新をCPU上で再現する。Unity版の生成数は50個/秒、寿命は20秒だが、three.js版では画面上の密度を調整した値を採用する。経過時間から生成イベント番号と生成時刻を直接求め、直近30秒以内のイベントだけを表示するため、定常表示数は生成頻度と寿命の積である450個となる。イベント番号を容量1000の描画枠へ順番に割り当て、枠の再利用時にも新しいイベント番号から位置、速度、色を決定する。位置と速度は画面座標へ正規化し、縦横比が変わっても画面全体へ分布させる。
- 元画像と紙吹雪をHalf Floatの中間ターゲットへ合成してから万華鏡処理へ渡し、紙吹雪の`1.0`を超える値をBloom前まで保持する。
- VFX Graph固有の3D姿勢は完全再現せず、平面矩形の回転と縦方向の縮みで紙片の回転を近似する。

## Unityからthree.jsへの対応

| Unity側 | three.js／Web側 |
| --- | --- |
...
- 強い発光や点滅に対する注意を、描画開始前に日本語と英語で表示する。

## 残りの検証

ビルド、単体テスト、ブラウザ確認の作業ルールは`AGENTS.md`に記載する。次の環境依存項目は自動テストだけで完結させず、実機で確認する。

- 前面カメラによる実時間の顔追跡
- HDR対応ディスプレイでのHDR出力と見え方
- SafariおよびiOS固有の動作
- 対象ブラウザでのWebGPU／WebGL 2フォールバック
- Unity版との目視比較
- 実機での性能と、開始前の注意表示

HDRディスプレイ出力の自動検証は必須としない。自動テストではHDR内部値、出力モードの分岐、SDRフォールバックまでを確認し、実際の輝度と見え方は実機で確認する。
