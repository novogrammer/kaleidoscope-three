...
- 本体の動画顔検出は初期値を15fpsとし、同じ動画フレームを重複して検出しない。
...
- 15fpsの顔中心を描画フレームごとに指数平滑化し、初期値として半減期`100ms`を使用する。初回検出時は非表示位置から移動して見えないよう、平滑化せず直ちに中心を合わせる。
...
従来の画像処理で使われてきたLena／Lenna画像は採用しない。元画像の再配布条件が明確なfixture向けライセンスではなく、被写体本人も利用終了を望んでいる。fixtureには画像生成AIで作成した架空人物の顔写真を使用する。正面顔の基準画像`src/assets/fixtures/face-center.jpg`、顔が中央から外れた`face-offset.jpg`、顔を含まない`no-face.jpg`を作成済みであり、生成方法とハッシュは`README.md`に記録する。
...
1. `CustomFunctions.hlsl`の座標処理を純粋なTypeScript関数へ移植し、CPU上の代表値テストを作る。
2. 検証済みの座標処理をTSLへ移植し、決定的なテスト用テクスチャで万華鏡表示を確認する。
3. 静止画fixtureとモック顔情報を接続し、3系統の万華鏡レイヤーを再現する。
4. HDR中間バッファ、Bloom、SDRトーンマッピングを構築する。
5. 紙吹雪の生成・更新・色・描画を再現する。
6. 静止画fixtureとMediaPipe Face Detectorを接続し、顔中心と座標変換を検証する。
7. モック入力で顔検出時の1秒フェードと中心移動を再現する。
8. 前面カメラ入力を追加し、実時間の顔追跡を手動確認する。
9. 対応環境でHDRディスプレイ出力を検証し、SDRフォールバックと表示状態を整える。
10. Unity版との比較、性能調整、安全面のUIを行う。
