...
開発中やCodexによるブラウザ検証では、Webカメラを必須にしない。カメラ権限、実行環境、照明、人物の位置による不安定さを避けるため、次の3種類の入力を切り替えられる構成にする。

| 入力モード | 用途 |
| --- | --- |
| 静止画fixture | MediaPipeの顔検出、アスペクト補正、左右反転、万華鏡との結合を再現可能な条件で確認する |
| モック顔情報 | MediaPipeを起動せず、固定またはスクリプト化した顔中心で描画、フェード、スクリーンショットを確認する |
| 前面カメラ | 権限取得、実時間の追従性、揺れ、実機性能を最終的に手動確認する |

通常の開発と軽い検証では静止画fixtureまたはモック顔情報を標準とし、前面カメラは明示的に選択したときだけ起動する。
...
MediaPipe、静止画fixture、モック入力のどれを使っても、描画側はこの共通データだけを参照する。モック入力では顔の出現・消失や中心座標の移動を時間列として与え、1秒フェードと追従を再現可能な形で検証する。
...
1. モック顔情報を使い、カメラとMediaPipeなしで紙吹雪と万華鏡の静的な基準画面を作る。
...
- 静止画fixtureまたはモック顔情報だけで、主要な描画を再現可能に検証できる。
